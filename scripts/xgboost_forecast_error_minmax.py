...
if __name__ == "__main__":
    ...
    for city in df.index.get_level_values("location").unique().tolist():
        ...
        # Perform scaling on y_test and test_preds
        y_test_array = y_test.values.reshape(-1, 1)  # Reshape for the scaler
        test_preds_array = test_preds.values.reshape(-1, 1)

        y_test_scaled = scaler.fit_transform(y_test_array)
        test_preds_scaled = scaler.transform(test_preds_array)

        # Calculate error after scaling
        error_scaled = y_test_scaled.flatten() - test_preds_scaled.flatten()

        # Convert the scaled error back to a pandas Series to retain the index
        errors[city] = pd.Series(error_scaled, index=y_test.index)
    ...
    plt.title("Error Comparison Among Cities with MinMax Scaling on Actual and Predicted", y=1.05)  # Adjust the title's vertical position
    plt.ylabel("Error (Actual - Predicted) with MinMax Scaling")
    plt.xlabel("Time")

    # Save the figure with 'bbox_inches' set to 'tight' to include all elements
    plt.savefig(str(out_dir / "error_minmax.png"), bbox_inches='tight')
    plt.clf()